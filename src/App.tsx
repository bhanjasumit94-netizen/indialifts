...
import { dbRefereeSessions, dbRefereeSignals, dbRefereeDevices } from "./lib/db";
...
const RefereePage = () => {
  ...
  const endAllRefereeSessions = async () => {
    if (!activeCompetitionId) {
      setSessionNotice("Select a competition first.");
      return;
    }
    ...
    const confirmed = typeof window !== "undefined"
      ? window.confirm("Disconnect all referee phones? Every scanned QR session will be invalidated.")
      : true;
    if (!confirmed) return;
    setSessionNotice("Ending all referee sessions...");
    try {
      await dbRefereeSessions.invalidateAll(activeCompetitionId);
      await dbRefereeSignals.clearAll(activeCompetitionId).catch(() => undefined);
      for (const pos of [0, 1, 2]) {
        await dbRefereeDevices.remove(activeCompetitionId, pos).catch(() => undefined);
      }
      setActiveSession(null);
      setSessionNotice("All referee phones disconnected. Create a new session to reconnect.");
    } catch (error) {
      console.error(LOG_SESSION, "failed to end referee sessions", error);
      setSessionNotice("Failed to end sessions. Please try again.");
    }
  };
          ...
          <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <button
                  type="button"
                  onClick={() => void endAllRefereeSessions()}
                  disabled={!activeCompetitionId || !isFirebaseConfigured || !activeSession}
                  className="rounded-xl bg-red-500 px-4 py-2 text-sm font-semibold text-white transition-opacity hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  End Sessions (Disconnect All)
                </button>
              </div>
            </div>
