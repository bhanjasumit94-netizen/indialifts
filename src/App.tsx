...
import { useFirebaseSync, type ConnectedRefereeSlots, type CompetitionSessionFromDb } from "./lib/useFirebaseSync";
import {
  type LiftType,
  type AttemptStatus,
  type TimerPhase,
...
type AppContextValue = {
  ...
  persistSessionSnapshot: (snapshot: import("./lib/useFirebaseSync").SessionPersistSnapshot) => Promise<void>;
  refetchSession: (reason?: string) => void;
};


const SYNC_KEY = "powerliftinglive.sync";
const DISPLAY_SETTINGS_KEY = "powerliftinglive.displaySettings";
...
function readSavedDisplaySettings(): { theme: DisplayThemeKey; scale: string; font: DisplayFontFamilyKey } {
  if (typeof window === "undefined") return { theme: "black", scale: "1", font: "system" };
  try {
    const raw = window.localStorage.getItem(DISPLAY_SETTINGS_KEY);
    if (!raw) return { theme: "black", scale: "1", font: "system" };
    const parsed = JSON.parse(raw) as Partial<{ theme: string; scale: string; font: string }>;
    return {
      theme: parseDisplayThemeFromSearch(parsed.theme ?? null) ?? "black",
      scale: DISPLAY_SCALE_OPTIONS.some((opt) => opt.value === parsed.scale) ? parsed.scale! : "1",
      font: parseDisplayFontFromSearch(parsed.font ?? null),
    };
  } catch {
    return { theme: "black", scale: "1", font: "system" };
  }
}
...
// Category header background colors (shared across display screens).
// Ordered most-specific first so `includes` matches correctly.
const CATEGORY_HEADER_COLORS: Array<[string, string]> = [
  ["SUB JUNIOR", "#00C853"],
  ["SUB-JUNIOR", "#00C853"],
  ["MASTERS 1",  "#8E24AA"],
  ["MASTERS 2",  "#D32F2F"],
...
const getCategoryHeaderColor = (name: string): string => {
  const upper = (name || "").toUpperCase();
  for (const [key, color] of CATEGORY_HEADER_COLORS) {
...
type EditableAttemptWeight = number | string | "";
...
const normalizeAttemptInputWeight = (value: EditableAttemptWeight): number | "" => {
  if (value === "") return "";
...
const getIndiaDistrictOnly = (team: string) => {
...
const isFinalAttemptStatus = (status: AttemptStatus | undefined) => status === "GOOD" || status === "NO";

const isFinalAttempt = (attempt: Attempt | null | undefined) => isFinalAttemptStatus(attempt?.status);
...
const findOfficialNextPlatformAttempt = (
  lifters: Lifter[],
  lift: LiftType,
  attemptIndex: number,
  competitionMode: CompetitionMode,
  manualOrderByStage: Record<string, string[]>,
): { lifterId: string; lift: LiftType; attemptIndex: number } | null => {
  const sequence = getStageSequence(competitionMode);
  const currentRank = sequence.findIndex((stage) => stage.lift === lift && stage.attemptIndex === attemptIndex);
  const startRank = currentRank >= 0 ? currentRank : 0;

  for (let idx = startRank; idx < sequence.length; idx += 1) {
    const stage = sequence[idx];
    const nextLifter = orderLiftersForDisplayRound(
      lifters,
      stage.lift,
      stage.attemptIndex,
      manualOrderByStage,
    ).find((lifter) => {
      const attempt = getAttempts(lifter, stage.lift)[stage.attemptIndex];
      return Boolean(attempt) && !isFinalAttempt(attempt) && getAttemptValue(lifter, stage.lift, stage.attemptIndex) !== null;
    });

    if (nextLifter) {
      return { lifterId: nextLifter.id, lift: stage.lift, attemptIndex: stage.attemptIndex };
    }
  }
...
const rebuildLiveCompetitionQueue = ({
  lifters,
  currentLift,
  currentAttemptIndex,
  competitionMode,
  ...
  const sessionLifters =
    activeCompetitionGroupName !== null
      ? lifters.filter((lifter) => isInGroup(lifter.group, activeCompetitionGroupName))
      : lifters;
  const platform = findOfficialNextPlatformAttempt(
    sessionLifters,
    currentLift,
    currentAttemptIndex,
    competitionMode,
  ...
  const rebuiltNextAttemptQueue = sortNextAttemptQueue(
    [...nextAttemptQueue, ...derivePendingNextAttemptQueue(sessionLifters, competitionMode)],
    lifters,
    competitionMode,
  ).filter((entry) => isPendingQueueEntry(entry, lifters));

  return {
    currentLifterId: platform?.lifterId ?? null,
    currentLift: platform?.lift ?? currentLift,
    currentAttemptIndex: platform?.attemptIndex ?? currentAttemptIndex,
    nextAttemptQueue: rebuiltNextAttemptQueue,
  };
};
...
const AppProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  /** Display screen is read-only when Firebase is not configured. */
  const firebaseSyncReadOnly = isDisplayScreen && !isFirebaseConfigured;
  ...
  const onCompetitionsLoaded = useCallback((loadedComps: CompetitionRecord[]) => {
    ...
    try { storedCid = localStorage.getItem("activeCompetitionId"); } catch {}
    ...
    const targetId =
      ...
      (storedCid && normalized.some((competition) => competition.id === storedCid) ? storedCid : null) ??
      normalized[0]?.id ??
      null;
    const target = targetId ? normalized.find((competition) => competition.id === targetId) : null;

    setCompetitionsState(normalized);
    setActiveCompetitionIdState(targetId ?? null);
    if (targetId) { try { localStorage.setItem("activeCompetitionId", targetId); } catch {} }

    if (!target) {
      // No competitions available — reset derived state to empty defaults.
      setLiftersState([]);
      liftersRef.current = [];
      setGroupsState(defaultGroups);
      setCurrentLifterIdState(null);
      currentLifterIdRef.current = null;
      currentLifterIdRef.current = null;
      setRefereeSignals([null, null, null]);
      setRefereeInputLockedState(false);
      setCurrentLiftState("squat");
      currentLiftRef.current = "squat";
      setCurrentAttemptIndexState(0);
      currentAttemptIndexRef.current = 0;
      setCompetitionStartedState(false);
      setIncludeCollarsState(false);
      setTimerPhaseState("IDLE");
      setTimerEndsAtState(null);
      setCompetitionModeState("FULL_GAME");
      competitionModeRef.current = "FULL_GAME";
      setNextAttemptQueueState([]);
      nextAttemptQueueRef.current = [];
      setActiveCompetitionGroupNameState(null);
      activeCompetitionGroupNameRef.current = null;
      setManualOrderByStageState({});
      manualOrderByStageRef.current = {};
      console.log(LOG_SESSION, "initial competitions loaded from DB (empty)", { competitionCount: 0 });
      return;
    }

    // If the active competition is unchanged, do NOT overwrite live lifter/group state
    // here — the per-competition realtime session listener (onCompetitionSessionFromDb)
    // is the single source of truth for lifters/groups and handles debounced updates.
    // Overwriting here causes a race where a just-added lifter can be wiped out by a
    // stale `lifters/{cid}` snapshot fetched before the lifter write has propagated.
    const isSameActiveCompetition = prevActiveId === target.id;
    if (!isSameActiveCompetition) {
      setLiftersState(target.lifters);
      liftersRef.current = target.lifters;
      setGroupsState(target.groups);
      setCurrentLifterIdState(target.currentLifterId ?? target.lifters[0]?.id ?? null);
      currentLifterIdRef.current = target.currentLifterId ?? target.lifters[0]?.id ?? null;
      setRefereeSignals([null, null, null]);
      setRefereeInputLockedState(target.refereeInputLocked);
      setCurrentLiftState(target.currentLift);
      currentLiftRef.current = target.currentLift;
      setCurrentAttemptIndexState(target.currentAttemptIndex);
      currentAttemptIndexRef.current = target.currentAttemptIndex;
      ...
      competitionModeRef.current = target.competitionMode;
      setNextAttemptQueueState(target.nextAttemptQueue);
      nextAttemptQueueRef.current = target.nextAttemptQueue;
      setActiveCompetitionGroupNameState(target.activeCompetitionGroupName ?? null);
      activeCompetitionGroupNameRef.current = target.activeCompetitionGroupName ?? null;
      setManualOrderByStageState(target.manualOrderByStage ?? {});
      manualOrderByStageRef.current = target.manualOrderByStage ?? {};
    }
    console.log(LOG_SESSION, "initial competitions loaded from DB", {
      ...
      preservedLocalState: isSameActiveCompetition,
    });
  }, []);
  ...
  const onCompetitionSessionFromDb = useCallback((session: CompetitionSessionFromDb) => {
    ...
    console.log(
      ...
      "session applied from DB (Control/Display UI updated)",
      {
        role: syncLogRole(),
        competitionId,
        currentLifterId: liveQueue.currentLifterId,
        currentLifterName: appliedName,
        currentLift: liveQueue.currentLift,
        attempt: liveQueue.currentAttemptIndex + 1,
        lifterCount: normalizedLifters.length,
      },
    );

    setLiftersState(normalizedLifters);
    liftersRef.current = normalizedLifters;
    setGroupsState(normalizedGroups);
    setCurrentLifterIdState(liveQueue.currentLifterId);
    currentLifterIdRef.current = liveQueue.currentLifterId;
    setCurrentLiftState(liveQueue.currentLift);
    currentLiftRef.current = liveQueue.currentLift;
    setCurrentAttemptIndexState(liveQueue.currentAttemptIndex);
    currentAttemptIndexRef.current = liveQueue.currentAttemptIndex;
    ...
    competitionModeRef.current = session.competitionMode;
    setNextAttemptQueueState(liveQueue.nextAttemptQueue);
    nextAttemptQueueRef.current = liveQueue.nextAttemptQueue;
    setActiveCompetitionGroupNameState(session.activeCompetitionGroupName);
    activeCompetitionGroupNameRef.current = session.activeCompetitionGroupName;
    setManualOrderByStageState(session.manualOrderByStage);
    manualOrderByStageRef.current = session.manualOrderByStage;
      ...
      prev.map((competition) =>
        competition.id === competitionId
          ? {
              ...
              currentLifterId: liveQueue.currentLifterId,
              currentLift: liveQueue.currentLift,
              currentAttemptIndex: liveQueue.currentAttemptIndex,
              ...
              nextAttemptQueue: liveQueue.nextAttemptQueue,
  ...
  const setActiveCompetitionGroupName = useCallback((name: string | null) => {
    activeCompetitionGroupNameRef.current = name;
  ...
  const setManualOrderByStage = useCallback((value: SetStateAction<Record<string, string[]>>) => {
    setManualOrderByStageState((prev) => {
      ...
      manualOrderByStageRef.current = next;
  ...
  const hydrateCompetition = (competition: CompetitionRecord | null) => {
    if (!competition) {
      ...
      liftersRef.current = empty.lifters;
      setGroupsState(empty.groups);
      setCurrentLifterIdState(empty.currentLifterId);
      currentLifterIdRef.current = empty.currentLifterId;
      ...
      currentLiftRef.current = empty.currentLift;
      setCurrentAttemptIndexState(empty.currentAttemptIndex);
      currentAttemptIndexRef.current = empty.currentAttemptIndex;
    ...
    competitionModeRef.current = competition.competitionMode;
    setNextAttemptQueueState(competition.nextAttemptQueue);
    nextAttemptQueueRef.current = competition.nextAttemptQueue;
    setActiveCompetitionGroupNameState(competition.activeCompetitionGroupName ?? null);
    activeCompetitionGroupNameRef.current = competition.activeCompetitionGroupName ?? null;
    setManualOrderByStageState(competition.manualOrderByStage ?? {});
    manualOrderByStageRef.current = competition.manualOrderByStage ?? {};
  };
  ...
  const applyIncomingState = useCallback((data: Partial<AppContextValue>) => {
    ...
    // With Firebase: ignore full competitions[] snapshots from cross-tab messages (stale). Still accept
    ...
    if (data.lifters) {
      const normalized = data.lifters.map((l) => normalizeLifter(l));
      liftersRef.current = normalized;
      setLiftersState(normalized);
    }
    if (data.groups) setGroupsState(data.groups.map((g) => normalizeGroup(g)));
    if (typeof data.currentLifterId !== "undefined") {
      currentLifterIdRef.current = data.currentLifterId;
      setCurrentLifterIdState(data.currentLifterId);
    }
    if (typeof data.refereeInputLocked === "boolean") setRefereeInputLockedState(data.refereeInputLocked);
    if (Array.isArray((data as { refereeSignals?: unknown }).refereeSignals)) {
      const sigs = (data as { refereeSignals?: RefSignal[] }).refereeSignals ?? [null, null, null];
      refereeSignalsRef.current = sigs;
      setRefereeSignalsState(sigs);
    }
    if (data.currentLift) {
      currentLiftRef.current = data.currentLift;
      setCurrentLiftState(data.currentLift);
    }
    if (typeof data.currentAttemptIndex === "number") {
      currentAttemptIndexRef.current = data.currentAttemptIndex;
      setCurrentAttemptIndexState(data.currentAttemptIndex);
    }
    ...
    if (data.competitionMode === "FULL_GAME" || data.competitionMode === "BENCH_ONLY") {
      competitionModeRef.current = data.competitionMode;
      setCompetitionModeState(data.competitionMode);
    }
    if (Array.isArray((data as { nextAttemptQueue?: unknown }).nextAttemptQueue)) {
      const q = (data as { nextAttemptQueue?: NextAttemptEntry[] }).nextAttemptQueue ?? [];
      nextAttemptQueueRef.current = q;
      setNextAttemptQueueState(q);
    }
    ...
    if (typeof patchGroup.activeCompetitionGroupName === "string" || patchGroup.activeCompetitionGroupName === null) {
      activeCompetitionGroupNameRef.current = patchGroup.activeCompetitionGroupName;
    ...
    if (typeof (data as { manualOrderByStage?: unknown }).manualOrderByStage !== "undefined") {
      ...
      if (mo && typeof mo === "object" && !Array.isArray(mo)) {
        manualOrderByStageRef.current = mo;
  ...
  const setNextAttemptQueue = useCallback((queue: NextAttemptEntry[]) => {
    nextAttemptQueueRef.current = queue;
  ...
  const switchCompetition = (competitionId: string) => {
    ...
    try { localStorage.setItem("activeCompetitionId", target.id); } catch {}
    if (isFirebaseConfigured) {
      // Session channel refetches from DB; avoid hydrating stale localStorage snapshot.
      console.log(LOG_CONTROL, "switch competition (Firebase — waiting for session refetch)", {
  ...
  const setCompetitionMode = (mode: CompetitionMode) => {
    competitionModeRef.current = mode;
  ...
  const updateAttemptForLifter = useCallback((lifterId: string, lift: LiftType, attemptIndex: number, weight: number | "") => {
    ...
    const focusedLifter = currentLifterId ? updated.find((row) => row.id === currentLifterId) : null;
    const focusedAttempt = focusedLifter ? getAttempts(focusedLifter, currentLift)[currentAttemptIndex] : null;
    const rebuiltQueue = isFinalAttempt(focusedAttempt)
    ...
    const queueChanged = JSON.stringify(rebuiltQueue.nextAttemptQueue) !== JSON.stringify(nextAttemptQueue);
    if (queueChanged) {
      setNextAttemptQueue(rebuiltQueue.nextAttemptQueue);
      if (timerPhase === "NEXT_ATTEMPT") {
        if (rebuiltQueue.nextAttemptQueue.length > 0) {
    ...
    console.log(LOG_SESSION, "NEXT_ATTEMPT_SAVED", {
      ...
      attemptNumber: attemptIndex + 1,
      weight,
      queueRemaining: rebuiltQueue.nextAttemptQueue.length,
      ts: Date.now(),
    });
    ...
    if (isFirebaseConfigured && !firebaseSyncReadOnly) {
      const nextTimerPhase: TimerPhase = queueChanged && timerPhase === "NEXT_ATTEMPT"
        ? (rebuiltQueue.nextAttemptQueue.length > 0 ? "NEXT_ATTEMPT" : "IDLE")
        : timerPhase;
      ...
      void persistSessionSnapshot({
        ...
        competitionMode,
        activeCompetitionGroupName,
        nextAttemptQueue: rebuiltQueue.nextAttemptQueue,
        manualOrderByStage,
      }).then(() => {
    ...
    setCurrentLifterId, setCurrentLift, setCurrentAttemptIndex,
  ...
  const applyRefereeDecision = useCallback(async (overrideSignals?: RefSignal[]) => {
    ...
    if (JSON.stringify(normalizedQueue) !== JSON.stringify(nextAttemptQueueNow)) {
      setNextAttemptQueue(normalizedQueue);
    }
  ...
  useEffect(() => {
    ...
    // Prevent re-firing verdict on the NEW (advanced) lifter/attempt when signals
    // haven't been cleared yet. If the current focused attempt already has a final
    // status, the verdict was just applied — wait for resetSignals() to clear.
    const focusedLifter = lifters.find((l) => l.id === currentLifterId);
    const focusedAttempt = focusedLifter ? getAttempts(focusedLifter, currentLift)[currentAttemptIndex] : null;
    if (focusedAttempt?.status === "GOOD" || focusedAttempt?.status === "NO") {
      return;
    }
  ...
  }, [refereeSignals, currentLifterId, currentLift, currentAttemptIndex, lifters, applyRefereeDecision]);
...
const navItems: { to: string; label: string; icon: string; requiresCompetition?: boolean; adminOnly?: boolean }[] = [
  { to: "/admin/users", label: "Admin Users", icon: "🛡️", adminOnly: true },
  { to: "/competitions", label: "Competitions", icon: "🏆" },
  { to: "/control", label: "Control Center", icon: "⚡" },
  { to: "/lifters", label: "Manage Lifters", icon: "👤", requiresCompetition: true },
  { to: "/groups", label: "Groups", icon: "👥", requiresCompetition: true },
  { to: "/signals", label: "Referee Signals", icon: "🏴" },
  { to: "/screen", label: "Display Screens", icon: "🖥" },
  { to: "/results", label: "Results", icon: "📊", requiresCompetition: true },
...
const ControlPage = () => {
  ...
  // Offline-only: auto-advance to first incomplete lifter in the round. With Firebase, current lifter comes from DB realtime.
  useEffect(() => {
    if (isFirebaseConfigured) return;
  ...
  return (
    <div
      className="flex min-h-[calc(100vh-4rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] text-white shadow-2xl"
      style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}
    >
      {/* ═══ TOP NAVBAR ═══ */}
      <header
        className="flex flex-shrink-0 items-center justify-between px-5 py-3"
        style={{ background: "rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.14)" }}
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px]" style={{ color: "#94a3b8" }}>Powerlifting Meet Platform</p>
          </div>
        </div>
        {/* Center: Group + Lift */}
        <div className="flex items-center gap-5">
          <div className="flex flex-col items-center rounded px-3 py-1" style={{ background: "rgba(255,255,255,0.1)" }}>
            <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: "#94a3b8" }}>GROUP</span>
            <span className="text-xl font-black text-white leading-none">{activeCompetitionGroupName || "ALL"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-lg">🏃</span>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: "#94a3b8" }}>CURRENT LIFT</p>
        ...
        <div className="flex items-center gap-5">
          <div className="text-right">
            <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: "#94a3b8" }}>TIME REMAINING</p>
            <p
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: "#94a3b8" }}>CONNECTION</p>
            <span
      ...
      <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
        ...
        {/* ── LEFT SIDEBAR ── */}
        <aside
          className="flex w-full flex-shrink-0 flex-col overflow-y-auto lg:w-60"
          style={{ background: "rgba(255,255,255,0.05)", borderRight: "1px solid rgba(255,255,255,0.14)" }}
        >



          {/* Meet Status */}
          <div className="flex-shrink-0 p-4" style={{ borderTop: "1px solid rgba(255,255,255,0.14)" }}>
            <p className="mb-3 text-[10px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>
              MEET STATUS
            </p>
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs" style={{ color: "#94a3b8" }}>Active Group</span>
                <span className="text-xs font-bold" style={{ color: "#00e676" }}>
                  {activeCompetitionGroupName || "All Groups"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs" style={{ color: "#94a3b8" }}>Lifters Remaining</span>
                <span className="text-xs font-bold text-white">{activeStageLifters.length} / {sessionLifters.length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs" style={{ color: "#94a3b8" }}>Platform</span>
          ...
          {/* Competition Mode */}
          <div className="flex-shrink-0 p-4" style={{ borderTop: "1px solid rgba(255,255,255,0.14)" }}>
            <p className="mb-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>
              COMPETITION MODE
            </p>
              ...
              <button
                ...
                style={
                  competitionMode === "FULL_GAME"
                    ...
                    : { background: "rgba(255,255,255,0.1)", color: "#94a3b8", border: "1px solid rgba(255,255,255,0.14)" }
                }
              >
                Full Game
              </button>
          ...
          <div className="flex-shrink-0 px-4 py-3">
            <p className="text-[10px]" style={{ color: "#94a3b8" }}>POWERLIFTING MEET CONSOLE v2.0.0</p>
          </div>
        </aside>
        ...
        <main className="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
          ...
          >
            ...
            <div className="mt-3 flex items-center justify-center gap-4 text-sm" style={{ color: "#94a3b8" }}>
            ...
            <div className="mt-3 flex items-center justify-center gap-2">
              ...
              <span className="mx-1 text-lg" style={{ color: "rgba(255,255,255,0.14)" }}>•</span>
              <span className="text-xl font-black" style={{ color: "#0891b2" }}>ATTEMPT {currentAttemptIndex + 1}</span>
            </div>
            ...
            <div className="mt-4 flex justify-center">
              <div
                ...
                style={{ background: "rgba(0,0,0,0.28)", border: "1px solid rgba(255,255,255,0.14)" }}
              >
                <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>BAR STATUS</span>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
              ...
              <label
                ...
                style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.14)", color: "#94a3b8" }}
              >
                <input
                  type="checkbox"
            ...
            {/* Referee Signals */}
            <div className="mt-5">
              <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "#94a3b8" }}>
                REFEREE SIGNALS
              </p>
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-2">
                  {["ATTEMPT 1", "ATTEMPT 2", "ATTEMPT 3"].map((label, i) => {
                    ...
                    return (
                      <div key={label} className="rounded-lg p-3 text-center" style={{ background: "rgba(0,0,0,0.28)", border: "1px solid rgba(255,255,255,0.14)" }}>
                        <p className="mb-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>{label}</p>
                        <p
                          className="text-base font-black"
                          style={{ color: attempt?.status === "GOOD" ? "#00e676" : attempt?.status === "NO" ? "#f44336" : "#94a3b8" }}
                        >
                  ...
                  <div
                    ...
                    style={{ background: "rgba(0,0,0,0.28)", border: `1px solid ${bestWeight !== null ? "#ffd700" : "rgba(255,255,255,0.14)"}` }}
                  >
          ...
          >
            ...
            <div className="space-y-3">
              {queuedAttemptRows.map(({ entry, lifter }, queueIndex) => {
                ...
                const applyTypedWeight = (rawValue: string) => {
                  const customValue = Number(rawValue.trim());
                  if (Number.isFinite(customValue) && customValue > 0) {
                    applyWeight(Number(customValue.toFixed(1)));
                  } else if (rawValue.trim() !== "") {
                    setActionNotice("Enter a valid number.");
                  }
                };
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${queueIndex === 0 ? "#0891b2" : "rgba(255,255,255,0.14)"}`,
                    }}
                  >
                      ...
                      <div className="flex flex-shrink-0 items-center gap-2">
                        {queueIndex === 0 && (
                          <span
                            ...
                            style={{ background: "rgba(8,145,178,0.18)", color: "#0891b2", border: "1px solid #0891b2" }}
                          >
                            CURRENT NEXT
                          </span>
                        )}
                        <span
                          ...
                          style={{ background: "rgba(255,255,255,0.1)", color: perLifterSignedSeconds < 0 ? "#f44336" : "#94a3b8", border: "1px solid rgba(255,255,255,0.14)" }}
                        >
                    ...
                    <div className="mt-3 grid grid-cols-5 gap-1.5">
                      {quickWeights.map((w) => (
                        <button
                          ...
                          style={{
                            background: attempt?.weight === w ? "#0891b2" : "rgba(255,255,255,0.1)",
                            color: "#ffffff",
                            border: `1px solid ${attempt?.weight === w ? "#0891b2" : "rgba(255,255,255,0.14)"}`,
                          }}
                        >
                          {w}
                        </button>
                    ...
                    <div className="mt-2 flex gap-2">
                      <input
                        ...
                        pattern="[0-9]*\.?[0-9]*"
                        value={draft}
                        onChange={(e) => setQuickWeightDraft((prev) => ({ ...prev, [draftKey]: e.target.value.replace(/[^0-9.]/g, "") }))}
                        onBlur={(e) => applyTypedWeight(e.currentTarget.value)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                            applyTypedWeight(e.currentTarget.value);
                          }
                        }}
              ...
              {queuedAttemptRows.length === 0 && (
                <p className="py-3 text-center text-sm" style={{ color: "#94a3b8" }}>
          ...
          >
            ...
            <input
              ...
              style={{ background: "rgba(0,0,0,0.28)", borderColor: "rgba(255,255,255,0.14)" }}
            />
                        ...
                        {editingOrderLifterId === lifter.id ? (
                          <div className="mt-2 grid grid-cols-3 gap-2">
                            <input
                              ...
                              onChange={(e) => setOrderEditAttempt(e.target.value.replace(/[^0-9.]/g, ""))}
                            ...
                            <select
                              ...
                              style={{ background: "rgba(0,0,0,0.28)", borderColor: "rgba(255,255,255,0.14)" }}
                            >
                              <option value="" style={{ background: "rgba(0,0,0,0.28)" }}>Lot</option>
                              {LOT_NUMBER_OPTIONS.map((lotNo) => (
                                <option key={lotNo} value={lotNo} style={{ background: "rgba(0,0,0,0.28)" }}>{lotNo}</option>
                              ))}
                            </select>
                          </div>
                        ) : (
        ...
        {/* ── RIGHT PANEL ── */}
        <aside
          className="flex w-full flex-shrink-0 flex-col gap-4 overflow-y-auto p-4 lg:w-80"
          style={{ borderLeft: "1px solid rgba(255,255,255,0.14)" }}
        >
          {/* Next Lifters */}
          <div className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.14)" }}>
            <div className="mb-3 flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>NEXT LIFTERS</p>
              <span className="text-xs font-semibold" style={{ color: "#0891b2" }}>View All</span>
            </div>
            <div className="space-y-2">
              {controlOrderLifters.slice(0, 5).map((lifter, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: lifter.id === currentLifterId ? "rgba(8,145,178,0.18)" : "rgba(255,255,255,0.1)",
                      border: `1px solid ${lifter.id === currentLifterId ? "#0891b2" : "rgba(255,255,255,0.14)"}`,
                    }}
                    onClick={() => {
                      const orderIndex = controlOrderLifters.findIndex((row) => row.id === lifter.id);
                  ...
                  >
                    <span
                      ...
                      style={{ background: "rgba(255,255,255,0.14)", color: "#94a3b8" }}
                    >
                      {i + 1}
                    </span>
                    <div className="min-w-0 flex-1">
                    ...
                    {nextWeight !== null && (
                      <span className="flex-shrink-0 text-sm font-black tabular-nums" style={{ color: "#0891b2" }}>
              ...
              {controlOrderLifters.length === 0 && (
                <p className="py-4 text-center text-sm" style={{ color: "#94a3b8" }}>No lifters in order.</p>
              )}
            </div>
          </div>
          ...
          {(() => {
            ...
            if (!firstQueue) {
              return (
                <div className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.14)" }}>
                  ...
                  <p className="py-4 text-center text-sm" style={{ color: "#94a3b8" }}>No pending declarations.</p>
                </div>
              );
            }
            ...
            const applyTypedQueueWeight = (rawValue: string) => {
              const customVal = Number(rawValue.trim());
              if (Number.isFinite(customVal) && customVal > 0) {
                applyQueueWeight(Number(customVal.toFixed(1)));
              } else if (rawValue.trim() !== "") {
                setActionNotice("Enter a valid number.");
              }
            };
            ...
            return (
              <div className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.14)" }}>
                <p className="mb-1 text-[10px] font-bold uppercase tracking-widest" style={{ color: "#94a3b8" }}>
                  ATTEMPT SELECTION
                </p>
                <p className="mb-3 text-xs font-semibold" style={{ color: "#0891b2" }}>
                ...
                <div className="mt-2 flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        applyTypedQueueWeight(e.currentTarget.value);
                      }
                    }}
                    ...
                    style={{ background: "rgba(0,0,0,0.28)", borderColor: "rgba(255,255,255,0.14)" }}
                  />
                  <span className="flex items-center px-1 text-base" style={{ color: "#94a3b8" }}>⌨</span>
                </div>
                <button
                  onClick={() => {
                    applyTypedQueueWeight(draft);
                  }}
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={label} className="flex items-start justify-between gap-2">
                  <span className="flex-shrink-0 text-xs" style={{ color: "#94a3b8" }}>{label}</span>
      ...
      <footer
        ...
        style={{ background: "rgba(255,255,255,0.05)", borderTop: "1px solid rgba(255,255,255,0.14)" }}
      >
        <div className="flex items-center gap-2">
...
const LifterManagementPage = () => {
  ...
  type LifterFormState = {
    ...
    lot: number | "";
    squat1: EditableAttemptWeight;
    bench1: EditableAttemptWeight;
    deadlift1: EditableAttemptWeight;
    isEquipped: boolean;
    disqualified: boolean;
  };
  const [form, setForm] = useState<LifterFormState>({
    name: "",
    sex: "Male",
    dob: "",
    bodyweight: "",
  ...
  const [bulkDrafts, setBulkDrafts] = useState<Record<string, LifterFormState>>({});
  const [selectedLifterIds, setSelectedLifterIds] = useState<string[]>([]);
  ...
  const buildDraftFromLifter = (l: Lifter): LifterFormState => ({
    name: l.name,
    sex: l.sex,
    dob: l.dob,
    bodyweight: l.bodyweight,
    manualWeightClass: l.manualWeightClass,
    category: l.category,
    group: Array.isArray(l.group) ? (l.group[0] ?? "") : l.group,
    team: l.team,
    rackHeightSquat: l.rackHeightSquat,
    rackHeightBench: l.rackHeightBench,
    lot: l.lot,
    squat1: l.squatAttempts[0]?.weight ?? "",
    bench1: l.benchAttempts[0]?.weight ?? "",
    deadlift1: l.deadliftAttempts[0]?.weight ?? "",
    isEquipped: l.isEquipped,
    disqualified: l.disqualified,
  });

  const enterEditMode = () => {
    const drafts: Record<string, LifterFormState> = {};
    lifters.forEach((l) => { drafts[l.id] = buildDraftFromLifter(l); });
  ...
  const saveAllChanges = () => {
    for (const id of Object.keys(bulkDrafts)) {
      if (!bulkDrafts[id].name.trim()) { setNotice("Lifter name is required."); return; }
    }
    const updated = lifters.map((l) => {
      const d = bulkDrafts[l.id];
      if (!d) return l;
      const opts = getCategoryOptions(d.sex);
      const category = opts.includes(d.category) ? d.category : opts[0];
      return {
        ...l,
        name: d.name.trim(),
        sex: d.sex,
        ...
        manualWeightClass: d.manualWeightClass.trim(),
        weightClass: resolveWeightClass(d.sex, d.bodyweight, d.manualWeightClass),
        category,
        group: d.group,
        team: d.team || "Independent",
        rackHeightSquat: d.rackHeightSquat,
        rackHeightBench: d.rackHeightBench,
        lot: d.lot,
        isEquipped: d.isEquipped,
        disqualified: d.disqualified,
        squatAttempts: keepAttemptWithFirstWeight(l.squatAttempts, d.squat1),
        benchAttempts: keepAttemptWithFirstWeight(l.benchAttempts, d.bench1),
        deadliftAttempts: keepAttemptWithFirstWeight(l.deadliftAttempts, d.deadlift1),
      };
    });
    setLifters(updated);
    setBulkDrafts({});
  ...
  const applyStateDistrictTeam = () => {
    ...
    setForm((prev) => ({ ...prev, team: teamDistrict }));
  };
  ...
  const keepAttemptWithFirstWeight = (attempts: Attempt[], firstWeight: EditableAttemptWeight) => {
    const next = [...attempts];
  ...
  const persistLifterSnapshot = (nextLifters: Lifter[], nextCurrentLifterId: string | null) => {
    void persistSessionSnapshot({
      lifters: nextLifters,
      groups,
      currentLifterId: nextCurrentLifterId,
      currentLift,
      currentAttemptIndex,
      competitionStarted,
      includeCollars,
      ...
      nextAttemptQueue,
      manualOrderByStage,
    }).catch((error) => console.error(LOG_CONTROL, "lifter snapshot persist failed", error));
  };
  ...
  const saveLifter = () => {
    ...
    if (editingId) {
      ...
      setLifters(updated);
      persistLifterSnapshot(updated, currentLifterId);
    ...
    const nextCurrentLifterId = currentLifterId ?? orderLiftersByIPF(updated, "squat", 0)[0]?.id ?? lifterToAdd.id;
    setLifters(updated);
    if (!currentLifterId) {
      setCurrentLifterId(nextCurrentLifterId);
    }
    persistLifterSnapshot(updated, nextCurrentLifterId);
  ...
  const editLifter = (lifter: Lifter) => {
    ...
    setForm({
      ...
      group: Array.isArray(lifter.group) ? lifter.group[0] : lifter.group,
      team: getIndiaDistrictOnly(lifter.team),
    ...
    const normalizedTeam = getIndiaDistrictOnly(lifter.team);
    const teamParts = lifter.team.split(" - ");
    if (teamParts.length >= 3 && teamParts[0] === "India") {
      const state = teamParts[1];
      const district = getIndiaDistrictOnly(lifter.team);
      ...
      for (const [state, districts] of Object.entries(INDIA_DISTRICTS)) {
        if (districts.includes(normalizedTeam)) {
          setTeamState(state);
          setTeamDistrict(normalizedTeam);
          break;
        }
      }
    }
  ...
  const deleteLifter = (lifter: Lifter) => {
    ...
    const nextCurrentLifterId = currentLifterId === lifter.id ? updated[0]?.id ?? null : currentLifterId;
    setLifters(updated);
    if (nextCurrentLifterId !== currentLifterId) setCurrentLifterId(nextCurrentLifterId);
    persistLifterSnapshot(updated, nextCurrentLifterId);
    if (editingId === lifter.id) resetForm();
    setSelectedLifterIds((prev) => prev.filter((id) => id !== lifter.id));
    setNotice(`Deleted ${lifter.name}.`);
  };

  const toggleLifterSelected = (id: string) => {
    setSelectedLifterIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  };

  const toggleSelectAllVisible = () => {
    const visibleIds = visibleLifters.map((l) => l.id);
    const allSelected = visibleIds.length > 0 && visibleIds.every((id) => selectedLifterIds.includes(id));
    if (allSelected) {
      setSelectedLifterIds((prev) => prev.filter((id) => !visibleIds.includes(id)));
    } else {
      setSelectedLifterIds((prev) => Array.from(new Set([...prev, ...visibleIds])));
    }
  };

  const bulkDeleteLifters = () => {
    if (selectedLifterIds.length === 0) return;
    const confirmed = window.confirm(`Delete ${selectedLifterIds.length} selected lifter(s)? This cannot be undone.`);
    if (!confirmed) return;
    const removeSet = new Set(selectedLifterIds);
    const updated = lifters.filter((l) => !removeSet.has(l.id));
    const nextCurrentLifterId = currentLifterId && removeSet.has(currentLifterId) ? updated[0]?.id ?? null : currentLifterId;
    setLifters(updated);
    if (nextCurrentLifterId !== currentLifterId) setCurrentLifterId(nextCurrentLifterId);
    persistLifterSnapshot(updated, nextCurrentLifterId);
    if (editingId && removeSet.has(editingId)) resetForm();
    setNotice(`Deleted ${removeSet.size} lifter(s).`);
    setSelectedLifterIds([]);
  };
          ...
          <div className={`grid gap-3 ${competitionMode === "BENCH_ONLY" ? "max-w-xs" : "md:grid-cols-3"}`}>
            {competitionMode !== "BENCH_ONLY" && (
              <Field
                ...
                placeholder="SQ 1"
                value={form.squat1}
                onChange={(e) => setForm((prev) => ({ ...prev, squat1: sanitizeDecimalInput(e.target.value) }))}
              />
            )}
            <Field
              type="text"
              ...
              placeholder="BP 1"
              value={form.bench1}
              onChange={(e) => setForm((prev) => ({ ...prev, bench1: sanitizeDecimalInput(e.target.value) }))}
            />
            {competitionMode !== "BENCH_ONLY" && (
              <Field
                type="text"
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, deadlift1: sanitizeDecimalInput(e.target.value) }))}
              />
            )}
          </div>
        </div>
      ...
      <div className="mt-5 overflow-x-auto rounded-2xl border border-white/15 bg-black/20">
        <div className="flex flex-wrap items-center gap-2 border-b border-white/10 px-3 py-3">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            {selectedLifterIds.length > 0 && !isEditMode && (
              <button
                onClick={bulkDeleteLifters}
                className="rounded-lg bg-red-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-400"
              >
                Delete Selected ({selectedLifterIds.length})
              </button>
            )}
            {isEditMode ? (
              <>
                ...
                <button
                  ...
                  className="rounded-lg bg-white/10 px-3 py-1.5 text-xs font-semibold text-slate-100 hover:bg-white/20"
                >
                  Cancel Changes
                </button>
              </>
              ...
              <button
                ...
                className="rounded-lg bg-purple-500/20 px-3 py-1.5 text-xs font-semibold text-purple-200 hover:bg-purple-500/30"
              >
                Edit Mode
              </button>
            )}
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-white/5 text-left text-slate-300">
            <tr>
              <th className="w-10 px-3 py-3">
                <input
                  type="checkbox"
                  aria-label="Select all lifters"
                  checked={
                    visibleLifters.length > 0 &&
                    visibleLifters.every((l) => selectedLifterIds.includes(l.id))
                  }
          ...
          <tbody>
            {visibleLifters.map((l) => {
              const isSingleEditing = editingId === l.id;
              const isEditing = isSingleEditing || isEditMode;
              const rowForm: LifterFormState = isSingleEditing
                ? form
                : (bulkDrafts[l.id] ?? buildDraftFromLifter(l));
              const updateRow = (patch: Partial<LifterFormState>) => {
                if (isSingleEditing) {
                  setForm((prev) => ({ ...prev, ...patch }));
                } else if (isEditMode) {
                  setBulkDrafts((prev) => ({
                    ...prev,
                    [l.id]: { ...(prev[l.id] ?? buildDraftFromLifter(l)), ...patch },
                  }));
                }
              };
              const rowResolvedWC = resolveWeightClass(rowForm.sex, rowForm.bodyweight, rowForm.manualWeightClass);
              return (
                <tr key={l.id} className="border-t border-white/10">
                  <td className="w-10 px-3 py-3">
                    <input
                      ...
                      checked={selectedLifterIds.includes(l.id)}
                      onChange={() => toggleLifterSelected(l.id)}
                    />
                  </td>
                    ...
                    {isEditing ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          updateRow({
                            sex: nextSex,
                            category: nextOptions.includes(rowForm.category) ? rowForm.category : nextOptions[0],
                          });
                        }}
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <input
                        type="date"
                        value={rowForm.dob}
                        onChange={(e) => updateRow({ dob: e.target.value })}
                        className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                      />
                    ) : (
                      l.dob || "-"
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <input
                        ...
                        step="0.05"
                        min={0}
                        value={rowForm.bodyweight}
                        onChange={(e) => updateRow({ bodyweight: e.target.value === "" ? "" : Number(e.target.value) })}
                        placeholder="kg"
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <select
                        ...
                        className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                      >
                        {getCategoryOptions(rowForm.sex).map((category) => (
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <div className="flex min-w-[14rem] flex-col gap-1">
                        ...
                        <select
                          value={MANUAL_WEIGHT_CLASSES.includes(rowForm.manualWeightClass) ? rowForm.manualWeightClass : ""}
                          onChange={(e) => updateRow({ manualWeightClass: e.target.value })}
                          className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        >
                          <option value="" className="bg-slate-900">
                            Auto from body weight
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <select
                        value={rowForm.group}
                        onChange={(e) => updateRow({ group: e.target.value })}
                        className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                      >
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      (() => {
                        const teamOpts = Array.from(new Set([
                          "Independent",
                          ...customTeams,
                          ...lifters.map((x) => x.team).filter(Boolean) as string[],
                        ]));
                        const teamVal = getIndiaDistrictOnly(rowForm.team || "");
                        let curState = "";
                        let curDistrict = "";
                        for (const [st, ds] of Object.entries(INDIA_DISTRICTS)) {
                          if ((ds as string[]).includes(teamVal)) {
                            curState = st;
                            curDistrict = teamVal;
                            break;
                          }
                        }
                        const districts = INDIA_DISTRICTS[curState] ?? [];
                        return (
                          <div className="flex min-w-56 flex-col gap-1">
                            <select
                              value={teamOpts.includes(rowForm.team) ? rowForm.team : "__custom__"}
                              onChange={(e) => {
                                if (e.target.value !== "__custom__") updateRow({ team: e.target.value });
                              }}
                              className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                            >
                              {teamOpts.map((t) => (
                              ...
                              <option value="__custom__" className="bg-slate-900">Custom / India…</option>
                            </select>
                            <div className="flex gap-1">
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  const d = (INDIA_DISTRICTS[s] ?? [""])[0] ?? "";
                                  updateRow({ team: d || "" });
                                }}
                                className="h-9 flex-1 rounded-lg border border-white/20 bg-black/40 px-1 text-xs"
                              >
                              ...
                              <select
                                value={curDistrict}
                                onChange={(e) => updateRow({ team: e.target.value })}
                                disabled={!curState}
                                className="h-9 flex-1 rounded-lg border border-white/20 bg-black/40 px-1 text-xs disabled:opacity-40"
                              >
                                <option value="" className="bg-slate-900">District</option>
                                {districts.map((d) => (
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <select
                        value={rowForm.lot}
                        onChange={(e) => updateRow({ lot: e.target.value === "" ? "" : Number(e.target.value) })}
                        className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                      >
                        <option value="" className="bg-slate-900">
                          Lot
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      <div className="flex gap-2">
                        <input
                          type="number"
                          value={rowForm.rackHeightSquat}
                          onChange={(e) => updateRow({ rackHeightSquat: e.target.value === "" ? "" : Number(e.target.value) })}
                          placeholder="S"
                          className="h-9 w-14 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        />
                        <input
                          type="number"
                          value={rowForm.rackHeightBench}
                          onChange={(e) => updateRow({ rackHeightBench: e.target.value === "" ? "" : Number(e.target.value) })}
                          placeholder="B"
                          className="h-9 w-14 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        />
                      </div>
                      ...
                      <div className="flex min-w-52 gap-2">
                        {competitionMode !== "BENCH_ONLY" && (
                          <input
                            ...
                            inputMode="decimal"
                            pattern="[0-9]*\.?[0-9]*"
                            value={rowForm.squat1}
                            onChange={(e) => updateRow({ squat1: sanitizeDecimalInput(e.target.value) })}
                            placeholder="SQ"
                        ...
                        <input
                          ...
                          inputMode="decimal"
                          pattern="[0-9]*\.?[0-9]*"
                          value={rowForm.bench1}
                          onChange={(e) => updateRow({ bench1: sanitizeDecimalInput(e.target.value) })}
                          placeholder="BP"
                          className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        />
                        {competitionMode !== "BENCH_ONLY" && (
                          <input
                            ...
                            inputMode="decimal"
                            pattern="[0-9]*\.?[0-9]*"
                            value={rowForm.deadlift1}
                            onChange={(e) => updateRow({ deadlift1: sanitizeDecimalInput(e.target.value) })}
                            placeholder="DL"
                            className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                        )}
            ...
            {visibleLifters.length === 0 && (
              <tr>
                <td colSpan={13} className="px-4 py-5 text-center text-slate-300">
...
const GroupManagementPage = () => {
  ...
  const [isEditMode, setIsEditMode] = useState(false);
  const [lifterDrafts, setLifterDrafts] = useState<Record<string, LifterDraft>>({});
  const [groupDrafts, setGroupDrafts] = useState<Record<string, string>>({});

  const keepAttemptWithFirstWeightLocal = (attempts: Attempt[], firstWeight: EditableAttemptWeight) => {
    const next = [...attempts];
    const weight = normalizeAttemptInputWeight(firstWeight);
    const existingStatus = next[0]?.status;
    const status: AttemptStatus = weight === "" ? "UNATTEMPTED" : (existingStatus ?? "UNATTEMPTED");
  ...
  const enterEditMode = () => {
    const lDrafts: Record<string, LifterDraft> = {};
    lifters.forEach((l) => {
      lDrafts[l.id] = {
        name: l.name,
        sex: l.sex,
    ...
    const gDrafts: Record<string, string> = {};
    groups.forEach((g) => { gDrafts[g.id] = g.name; });
    setLifterDrafts(lDrafts);
  ...
  const updateLifterDraft = (id: string, patch: Partial<LifterDraft>) => {
    setLifterDrafts((prev) => ({ ...prev, [id]: { ...prev[id], ...patch } }));
  };

  const updateGroupDraft = (id: string, name: string) => {
    setGroupDrafts((prev) => ({ ...prev, [id]: name }));
  };

  const saveAllChanges = () => {
    // Validate lifters
    for (const l of lifters) {
      const d = lifterDrafts[l.id];
      if (!d) continue;
      if (!d.name.trim()) { showNotice(`Lifter name is required.`, "error"); return; }
    }
    // Validate groups: no empty, no duplicates
    const seen = new Set<string>();
    for (const g of groups) {
      const nextName = (groupDrafts[g.id] ?? g.name).trim().toUpperCase();
      if (!nextName) { showNotice("Group name cannot be empty.", "error"); return; }
      if (seen.has(nextName)) { showNotice(`Duplicate group name "${nextName}".`, "error"); return; }
      seen.add(nextName);
    }
    // Build rename map
    ...
    const newGroups = groups.map((g) => {
      const nextName = (groupDrafts[g.id] ?? g.name).trim().toUpperCase();
      if (nextName !== g.name) renameMap[g.name] = nextName;
      return { ...g, name: nextName };
    });
    // Apply lifter drafts + rename map to group refs
    const applyRename = (grp: string | string[]) => {
      if (Array.isArray(grp)) {
        const mapped = grp.map((gn) => renameMap[gn] ?? gn);
        return mapped.length === 1 ? mapped[0] : mapped;
      }
      return renameMap[grp] ?? grp;
    };
    const newLifters = lifters.map((l) => {
      const d = lifterDrafts[l.id];
      if (!d) return { ...l, group: applyRename(l.group) };
      const isMulti = Array.isArray(l.group) && l.group.length > 1;
      const nextGroup = isMulti
        ? applyRename(l.group)
        : (renameMap[d.group] ?? d.group);
      return {
        ...l,
        name: d.name.trim(),
        sex: d.sex,
        ...
        rackHeightSquat: d.rackHeightSquat,
        rackHeightBench: d.rackHeightBench,
        squatAttempts: keepAttemptWithFirstWeightLocal(l.squatAttempts, d.squat1),
        benchAttempts: keepAttemptWithFirstWeightLocal(l.benchAttempts, d.bench1),
        deadliftAttempts: keepAttemptWithFirstWeightLocal(l.deadliftAttempts, d.deadlift1),
        dob: d.dob,
        isEquipped: d.isEquipped,
        disqualified: d.disqualified,
      };
    ...
    if (renameMap[selectedGroupName]) setSelectedGroupName(renameMap[selectedGroupName]);
    if (renameMap[activeGroupFilter]) setActiveGroupFilter(renameMap[activeGroupFilter]);
    if (renameMap[bulkTargetGroupName]) setBulkTargetGroupName(renameMap[bulkTargetGroupName]);
    setLifterDrafts({});
    setGroupDrafts({});
    setIsEditMode(false);
    showNotice("All changes saved.");
  };

  const deleteGroup = (group: Group) => {
    const fallbackGroup = groups.find((g) => g.id !== group.id);
    const updatedGroups = groups.filter((g) => g.id !== group.id);
    // Delete all lifters that belong ONLY to this group. Lifters shared with
    // other groups are kept and simply have this group removed from their list.
    const updatedLifters = lifters.reduce<Lifter[]>((acc, l) => {
      const groupArray = getGroupArray(l.group);
      if (!groupArray.includes(group.name)) {
        acc.push(l);
        return acc;
      }
      const remaining = groupArray.filter((g) => g !== group.name);
      if (remaining.length === 0) return acc; // drop lifter entirely
      acc.push(remaining.length === 1 ? { ...l, group: remaining[0] } : { ...l, group: remaining });
      return acc;
    }, []);
    const removedLifterCount = lifters.length - updatedLifters.length;
    setGroups(updatedGroups);
    setLifters(updatedLifters);
    if (currentLifterId && !updatedLifters.some((l) => l.id === currentLifterId)) {
      setCurrentLifterId(updatedLifters[0]?.id ?? null);
    }
    ...
    if (activeCompetitionGroupName === group.name) setActiveCompetitionGroupName(null);
    setGroupDrafts((prev) => { const next = { ...prev }; delete next[group.id]; return next; });
    setConfirmDeleteGroupId(null);
    showNotice(
      removedLifterCount > 0
        ? `Group "${group.name}" and ${removedLifterCount} lifter(s) deleted.`
  ...
  const toggleSelectAllGroups = () => {
    ...
    const allSelected = visibleIds.length > 0 && visibleIds.every((id) => selectedGroupIds.includes(id));
    if (allSelected) {
  ...
  const bulkDeleteGroups = () => {
    ...
    const targetNames = new Set(targets.map((g) => g.name));
    const liftersToRemove = lifters.filter((l) => {
      const arr = getGroupArray(l.group);
      return arr.length > 0 && arr.every((n) => targetNames.has(n));
    });
    const confirmed = window.confirm(
      `Delete ${targets.length} group(s)? All ${liftersToRemove.length} lifter(s) in these groups will also be permanently deleted.`,
    );
    if (!confirmed) return;
    const updatedGroups = groups.filter((g) => !selectedGroupIds.includes(g.id));
    const updatedLifters = lifters.reduce<Lifter[]>((acc, l) => {
      const arr = getGroupArray(l.group);
      const remaining = arr.filter((n) => !targetNames.has(n));
      if (arr.length > 0 && remaining.length === 0) return acc;
    ...
    if (activeCompetitionGroupName && targetNames.has(activeCompetitionGroupName)) {
      setActiveCompetitionGroupName(null);
    }
    if (activeGroupFilter && targetNames.has(activeGroupFilter)) setActiveGroupFilter("");
    if (selectedGroupName && targetNames.has(selectedGroupName)) {
      setSelectedGroupName(updatedGroups[0]?.name ?? "");
    }
    setSelectedGroupIds([]);
    setConfirmDeleteGroupId(null);
    showNotice(`Deleted ${targets.length} group(s) and ${lifters.length - updatedLifters.length} lifter(s).`);
  };
    ...
    <section className="space-y-5">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            {filteredGroups.length > 0 && !isEditMode && (
              <label className="flex cursor-pointer items-center gap-2 rounded-lg bg-white/5 px-3 py-1.5 text-xs text-slate-200">
                <input
                  type="checkbox"
                  checked={filteredGroups.every((g) => selectedGroupIds.includes(g.id))}
                  onChange={toggleSelectAllGroups}
                />
                Select all
              </label>
            )}
            {selectedGroupIds.length > 0 && !isEditMode && (
              <button
        ...
        <div className="space-y-3">
          {filteredGroups.map((group) => {
            ...
            const isEditingThis = isEditMode;
                    ...
                    >
                      <div className="mx-4 mb-4 rounded-xl border border-red-500/30 bg-red-900/20 p-4">
                        <p className="mb-3 text-sm text-red-200">
                          ...
                          {groupLifterCount > 0
                            ? `All ${groupLifterCount} lifter(s) in this group will also be permanently deleted.`
                            : "This group has no lifters."}
                        </p>
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            {isEditMode ? (
              <>
                ...
                <button
                  onClick={cancelAllChanges}
                  className="rounded-lg bg-white/10 px-3 py-1.5 text-xs font-semibold text-slate-200 hover:bg-white/15"
                >
                  Cancel Changes
                </button>
              </>
              ...
              <button
                onClick={enterEditMode}
                className="rounded-lg bg-purple-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-purple-400"
              >
                Edit Mode
              </button>
            )}
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-white/8 bg-white/[0.03]">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Lot</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Rack S/B</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Openers</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">BW / Sex</th>
              </tr>
            </thead>
            <tbody>
              {visibleLifters.map((l, idx) => {
                const draft = lifterDrafts[l.id];
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium text-white">
                      {isEditing ? (
                        <input
                          value={draft.name}
                          onChange={(e) => updateLifterDraft(l.id, { name: e.target.value })}
                    ...
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <select
                          value={draft.group}
                          onChange={(e) => updateLifterDraft(l.id, { group: e.target.value })}
                        ...
                        >
                          ...
                          {groups.map((g) => {
                            const gName = groupDrafts[g.id] ?? g.name;
                            return <option key={g.id} value={gName} className="bg-slate-900">{gName}</option>;
                          })}
                        </select>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <select
                          ...
                          onChange={(e) => updateLifterDraft(l.id, { category: e.target.value })}
                          className="h-9 min-w-40 rounded-lg border border-white/20 bg-black/40 px-2 text-sm focus:outline-none"
                        >
                          {getCategoryOptions(draft.sex).map((cat) => <option key={cat} value={cat} className="bg-slate-900">{cat}</option>)}
                        </select>
                    ...
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <div className="flex flex-col gap-1">
                          <span className="text-xs text-slate-400">Auto: {getIPFWeightClass(draft.sex, draft.bodyweight) || "—"}</span>
                          <input
                            value={draft.manualWeightClass}
                            onChange={(e) => updateLifterDraft(l.id, { manualWeightClass: e.target.value })}
                            placeholder="Manual override"
                            className="h-8 w-24 rounded-lg border border-white/20 bg-black/40 px-2 text-xs focus:outline-none"
                          />
                        </div>
                      ) : (l.weightClass || "—")}
                    </td>
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        (() => {
                          const teamOpts = Array.from(new Set([
                            "Independent",
                            ...lifters.map((x) => getIndiaDistrictOnly(x.team)).filter(Boolean) as string[],
                          ]));
                          const draftTeamDistrict = getIndiaDistrictOnly(draft.team || "");
                          let curState = "";
                          let curDistrict = "";
                          for (const [state, districtsForState] of Object.entries(INDIA_DISTRICTS)) {
                            if (districtsForState.includes(draftTeamDistrict)) {
                              curState = state;
                              curDistrict = draftTeamDistrict;
                              break;
                            }
                          }
                          const districts = INDIA_DISTRICTS[curState] ?? [];
                          return (
                            <div className="flex min-w-56 flex-col gap-1">
                              <select
                                value={teamOpts.includes(draftTeamDistrict) ? draftTeamDistrict : "__custom__"}
                                onChange={(e) => {
                                  if (e.target.value !== "__custom__") updateLifterDraft(l.id, { team: e.target.value });
                                }}
                                className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                              >
                                {teamOpts.map((t) => (
                              ...
                              <div className="flex gap-1">
                                ...
                                <select
                                  value={curDistrict}
                                  onChange={(e) => updateLifterDraft(l.id, { team: e.target.value })}
                                  disabled={!curState}
                                  className="h-9 flex-1 rounded-lg border border-white/20 bg-black/40 px-1 text-xs disabled:opacity-40"
                                >
                                  <option value="" className="bg-slate-900">District</option>
                      ...
                      ) : <span className="text-xs">{getIndiaDistrictOnly(l.team) || "—"}</span>}
                    </td>
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <select
                          value={draft.lot}
                          onChange={(e) => updateLifterDraft(l.id, { lot: e.target.value === "" ? "" : Number(e.target.value) })}
                          className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        >
                          <option value="" className="bg-slate-900">Lot</option>
                          {LOT_NUMBER_OPTIONS.map((n) => (
                            <option key={n} value={n} className="bg-slate-900">{n}</option>
                          ))}
                        </select>
                      ) : <span className="text-xs">{l.lot || "—"}</span>}
                    </td>
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <div className="flex gap-1">
                          <input
                            type="number"
                            value={draft.rackHeightSquat}
                            onChange={(e) => updateLifterDraft(l.id, { rackHeightSquat: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="S"
                            className="h-9 w-14 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                          <input
                            type="number"
                            value={draft.rackHeightBench}
                            onChange={(e) => updateLifterDraft(l.id, { rackHeightBench: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="B"
                            className="h-9 w-14 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                        </div>
                      ) : <span className="text-xs">{l.rackHeightSquat || "—"} / {l.rackHeightBench || "—"}</span>}
                    </td>
                    <td className="px-4 py-3 text-slate-300">
                      {isEditing ? (
                        <div className="flex gap-1">
                          {competitionMode !== "BENCH_ONLY" && (
                            <input
                              ...
                              pattern="[0-9]*\.?[0-9]*"
                              value={draft.squat1}
                              onChange={(e) => updateLifterDraft(l.id, { squat1: sanitizeDecimalInput(e.target.value) })}
                              placeholder="SQ"
                              className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                            />
                          )}
                          <input
                            ...
                            pattern="[0-9]*\.?[0-9]*"
                            value={draft.bench1}
                            onChange={(e) => updateLifterDraft(l.id, { bench1: sanitizeDecimalInput(e.target.value) })}
                            placeholder="BP"
                            className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                          {competitionMode !== "BENCH_ONLY" && (
                            <input
                              ...
                              inputMode="decimal"
                              pattern="[0-9]*\.?[0-9]*"
                              value={draft.deadlift1}
                              onChange={(e) => updateLifterDraft(l.id, { deadlift1: sanitizeDecimalInput(e.target.value) })}
                              placeholder="DL"
                              className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                            />
                          )}
                    ...
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <div className="flex flex-wrap gap-2">
                          <select
                            value={draft.sex}
                            onChange={(e) => updateLifterDraft(l.id, { sex: e.target.value as "Male" | "Female" })}
                          ...
                          <input
                            ...
                            onChange={(e) => updateLifterDraft(l.id, { bodyweight: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="BW kg"
                          ...
                          <input
                            type="date"
                            value={draft.dob}
                            onChange={(e) => updateLifterDraft(l.id, { dob: e.target.value })}
                            className="h-9 rounded-lg border border-white/20 bg-black/40 px-2 text-xs focus:outline-none"
                          />
                          <label className="flex items-center gap-1 text-xs text-slate-300">
                            <input
                              type="checkbox"
                              checked={draft.isEquipped}
                              onChange={(e) => updateLifterDraft(l.id, { isEquipped: e.target.checked })}
                              className="h-4 w-4 accent-cyan-400"
                            />
                            Equipped
                          </label>
                          <label className="flex items-center gap-1 text-xs text-slate-300">
                            <input
                              ...
                              onChange={(e) => updateLifterDraft(l.id, { disqualified: e.target.checked })}
                        ...
                        <span className="text-xs text-slate-500">{l.sex[0]} · {l.bodyweight || "—"}</span>
                      )}
                    </td>
                  </tr>
                );
...
const RefereePage = () => {
  ...
  const { competitions, activeCompetitionId, switchCompetition, connectedRefereeSlots, resetSignals } = useAppContext();
  ...
  const endRefereeSessions = async () => {
    if (!activeCompetitionId) {
      setSessionNotice("Select a competition first.");
      return;
    }
    if (!isFirebaseConfigured) {
      setSessionNotice("Firebase is not configured.");
      return;
    }
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => void endRefereeSessions()}
                disabled={!activeCompetitionId || !isFirebaseConfigured || !activeSession}
                className="rounded-xl bg-red-500 px-4 py-2 text-sm font-semibold text-white transition-opacity hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
                End Sessions
              </button>
            </div>
...
const RefereeStationPage = () => {
  ...
  // iOS Safari can restore this page from bfcache with a stale/empty context.
  // Force a fresh session pull whenever we have a competition but no current
  // lifter loaded yet, and once on mount.
  useEffect(() => {
    if (!activeCompetitionId) return;
    refetchSession("referee:mount");
  }, [activeCompetitionId, refetchSession]);

  useEffect(() => {
    if (!activeCompetitionId) return;
    if (currentLifterId && lifters.some((l) => l.id === currentLifterId)) return;
    const t = window.setTimeout(() => refetchSession("referee:no-current-lifter"), 400);
    return () => window.clearTimeout(t);
  }, [activeCompetitionId, currentLifterId, lifters, refetchSession]);
  ...
  const startDecisionHold = (decision: Exclude<RefSignal, null>, event: { preventDefault: () => void }) => {
    ...
    holdTimeoutRef.current = window.setTimeout(() => {
      ...
      // Optimistic local update is only needed when we're not syncing via Firebase.
      // In Firebase mode, we rely on realtime `refereeSignals` updates to avoid
...
const ScreenPage = () => {
  ...
  const savedDisplaySettings = useMemo(() => readSavedDisplaySettings(), []);
  const [screenType, setScreenType] = useState("signal_results_plate");
  const [displayVenueTheme, setDisplayVenueTheme] = useState<DisplayThemeKey>(savedDisplaySettings.theme);
  const [displayVenueScale, setDisplayVenueScale] = useState(savedDisplaySettings.scale);
  const [displayVenueFont, setDisplayVenueFont] = useState<DisplayFontFamilyKey>(savedDisplaySettings.font);

  useEffect(() => {
    localStorage.setItem(
      DISPLAY_SETTINGS_KEY,
      JSON.stringify({ theme: displayVenueTheme, scale: displayVenueScale, font: displayVenueFont }),
    );
  }, [displayVenueTheme, displayVenueScale, displayVenueFont]);

  const openDisplayScreen = () => {
    const activeCompetitionName =
      competitions.find((c) => c.id === activeCompetitionId)?.name ?? "Competition";
    // Live display reads referee signals from Firebase realtime — do not embed
...
const ResultsPage = () => {
  ...
  const updateAttemptCell = (
    ...
    const finalResultSaved = isFinalAttempt(nextAttempt);
    const rebuiltQueue = finalResultSaved
      ? rebuildLiveCompetitionQueue({
          lifters: updated,
          currentLift,
          currentAttemptIndex,
    ...
    void persistSessionSnapshot({
      ...
      console.log(LOG_SESSION, "Results attempt persisted", {
        lifterId,
        lift,
        attemptNumber: attemptIndex + 1,
        weight: nextAttempt.weight,
        status: nextAttempt.status,
      });
    }).catch((err) => console.error(LOG_SESSION, "Results attempt persist failed", err));
    return { ok: true, message: "Attempt updated." };
  };

  const getDraftKey = (lifterId: string, lift: LiftType, attemptIndex: number) => `${lifterId}-${lift}-${attemptIndex}`;

  const commitAttemptWeight = (
    lifterId: string,
    lift: LiftType,
    attemptIndex: number,
    ...
    const rawDraft = typeof rawValue === "string" ? rawValue : attemptDrafts[key];
              ...
              .map((r, idx) => {
                const renderEditor = (lift: LiftType, attemptIndex: number) => {
                  const attempt = getAttempts(r, lift)[attemptIndex] ?? { weight: "", status: "UNATTEMPTED" as AttemptStatus };
                    const draftKey = getDraftKey(r.id, lift, attemptIndex);
                  const statusClass =
                    attempt.status === "GOOD"
                      ? "bg-green-500/20"
                      : attempt.status === "NO"
                    ...
                    <td key={`${r.id}-${lift}-${attemptIndex}`} className="px-2 py-2 align-top">
                      <div className={`rounded-lg border border-white/10 p-2 ${statusClass}`}>
                        <input
                          ...
                          pattern="[0-9]*\.?[0-9]*"
                          value={
                            attemptDrafts[draftKey] ??
                            (attempt.weight === "" ? "" : String(attempt.weight))
                          }
                          onChange={(e) => {
                          ...
                          onBlur={(e) => {
                            ...
                            if (nextTarget?.dataset.attemptStatusSelect === draftKey) return;
                            commitAttemptWeight(r.id, lift, attemptIndex, attempt.weight, e.currentTarget.value);
                          }}
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              commitAttemptWeight(r.id, lift, attemptIndex, attempt.weight, e.currentTarget.value);
                        ...
                        />
                        <select
                          data-attempt-status-select={draftKey}
                          value={attempt.status}
                          onChange={(e) => {
                            const draftValue = attemptDrafts[draftKey];
                            const patch: Partial<Attempt> = { status: e.target.value as AttemptStatus };
                            ...
                            if (typeof draftValue !== "undefined") {
                              ...
                              if (normalized === "") {
                                ...
                                patch.weight = Number(parsed.toFixed(1));
                              }
                            }

                            const result = updateAttemptCell(r.id, lift, attemptIndex, patch);
                            setNotice(result.message);
                            if (result.ok && typeof draftValue !== "undefined") {
                              setAttemptDrafts((prev) => {
                                const { [draftKey]: _ignored, ...rest } = prev;
                                return rest;
                              });
                            }
                          }}
    ...
    return (
      ...
      >
        ...
        <td
          className={`align-top whitespace-normal break-words px-2 py-2 hidden md:table-cell ${isDarkTheme ? "text-slate-400" : "text-slate-600"}`}
        >
          {getIndiaDistrictOnly(lifter.team) || "-"}
        </td>
        <td
  ...
  return (
    <div className={`h-full space-y-3 overflow-y-auto ${DISPLAY_RESULTS_BODY}`}>
      {rankingByGroup.map(({ groupName, members }) => (
        ...
        >
          <div
            className="flex w-full items-center gap-3 border-b-2 border-black/10 px-4 py-4"
            style={{ backgroundColor: getCategoryHeaderColor(groupName) }}
          >
            <div className="h-3.5 w-1.5 rounded-full bg-white/80" />
            <div className="flex-1">
              <p className="text-sm font-black uppercase leading-tight tracking-[0.24em] text-white">
                {groupName || "Unassigned"}
              </p>
              <p className="mt-0.5 text-xs font-semibold normal-case tracking-normal text-white/80">
...
const NextAttemptQueueDisplay = ({
  forceLive,
  displayRootStyle,
  themeRootClass = "bg-[#0d0d0d] text-white",
  themeTone = "dark",
}: {
  forceLive: boolean;
  ...
  return (
    <div
      data-display-theme={themeTone}
      className={`w-full min-h-screen overflow-y-auto ${themeRootClass}`}
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                <input
                  ...
                  value={draft}
                  onChange={(e) =>
                    setQuickWeightDraft((prev) => ({ ...prev, [draftKey]: e.target.value.replace(/[^0-9.]/g, "") }))
                  }
                  onBlur={(e) => applyTypedWeight(e.currentTarget.value)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
...
const DisplayFullPage = () => {
  ...
  const savedDisplaySettings = useMemo(() => readSavedDisplaySettings(), []);
  const displayTheme = parseDisplayThemeFromSearch(searchParams.get("theme")) ?? savedDisplaySettings.theme;
  ...
  /** Set synchronously when verdict timers start; Firebase may clear rows before `isFinalVerdictAnimating` commits. */
  ...
  // With Firebase: display records the official verdict (Control only drives platform via DB realtime).
  useEffect(() => {
    if (!isFirebaseConfigured) return;
    ...
    // Guard: if the current attempt already has a final verdict, the decision was
    // just applied and the pointer advanced. Wait for signals to reset — do NOT
    // re-fire the verdict on the next lifter/attempt.
    const focusedLifter = lifters.find((l) => l.id === currentLifterId);
    const focusedAttempt = focusedLifter ? getAttempts(focusedLifter, currentLift)[currentAttemptIndex] : null;
    if (focusedAttempt?.status === "GOOD" || focusedAttempt?.status === "NO") {
      return;
  ...
  const displayRootStyle: CSSProperties = useMemo(
    () => ({
      "--display-bg": activeTheme.bg,
      "--display-fg": activeTheme.fg,
      "--display-surface": activeTheme.tone === "light" ? "#ffffff" : activeTheme.bg,
      "--display-surface-2": activeTheme.tone === "light" ? "#f1f5f9" : "rgba(255,255,255,0.06)",
    ...
    [displayFontKey, activeTheme.bg, activeTheme.fg, activeTheme.tone],
  );
  ...
  // Single source of truth for lifter order — same IPF rules used by Control,
  // Results, Referee, and Display screens. Fallbacks below must use this list,
  // never a lot-only sort, so every surface points at the same "current" lifter.
  const sortedLifters = useMemo(
    () => orderLiftersForFlightLineStrip(lifters, currentLift, currentAttemptIndex, manualOrderByStage),
    [lifters, currentLift, currentAttemptIndex, manualOrderByStage],
  );
  ...
  const platformTimerChip =
    timerPhase === "ATTEMPT" && timerEndsAt ? (
      <div
        className={`pointer-events-none fixed left-4 bottom-10 z-[45] max-w-[min(96vw,22rem)] rounded-2xl border-2 px-6 py-5 shadow-xl ${displayMode === "signal_results" ? "" : "backdrop-blur-md"} ${
          isDarkTheme
  ...
  useEffect(() => {
    ...
    if (!allSignalsReceived) {
      ...
      const completeAnimation = async () => {
        ...
        // applyRefereeDecision already clears signals when Firebase is on; keep for offline display.
        if (!isFirebaseConfigured) {
          void resetSignals();
        }
        setIsFinalVerdictAnimating(false);
  ...
  const showResultsInlineLiftingOrder = displayMode !== "order_attempts" && displayMode !== "signal_results" && flightLineOrdered.length > 0;
  ...
  if (displayMode === "ipf_plate") {
    ...
    return (
      <div
        data-display-theme={activeTheme.tone}
        className={`w-full overflow-hidden flex flex-col ${activeTheme.rootClass}`}
        style={{ ...displayRootStyle, height: "100vh" }}
      >
        {!competitionStarted && !forceLive && (
  ...
  if (displayMode === "bar_loading_tables") {
    ...
    return (
      <div
        data-display-theme={activeTheme.tone}
        className={`w-full overflow-hidden flex flex-col select-none ${activeTheme.rootClass}`}
        style={{ ...displayRootStyle, height: "100vh", fontFamily: "'Arial Narrow', 'Arial', sans-serif" }}>
  ...
  if (displayMode === "next_attempt_queue") {
    return <NextAttemptQueueDisplay forceLive={forceLive} displayRootStyle={displayRootStyle} themeRootClass={activeTheme.rootClass} themeTone={activeTheme.tone} />;
  }
  ...
  if (["signal_results_plate", "signal_results", "order_attempts", "results_all"].includes(displayMode)) {
    ...
    return (
      <div
        data-display-theme={activeTheme.tone}
        className={`relative flex h-screen w-screen flex-col overflow-hidden ${activeTheme.rootClass}`}
        style={displayRootStyle}
      >
        ...
        <div className={displayMode === "signal_results" ? "min-h-0 flex-1 overflow-hidden p-0" : "min-h-0 flex-1 overflow-auto px-3 py-2 md:px-5 md:py-3"}>
  ...
  return (
    <div data-display-theme={activeTheme.tone} className={`relative min-h-screen px-4 py-5 ${activeTheme.rootClass}`} style={displayRootStyle}>
      {!competitionStarted && !forceLive && (
        <div
