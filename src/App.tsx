...
const LifterManagementPage = () => {
  ...
  const bulkDeleteLifters = () => {
    ...
    setLifters(updated);
    if (currentLifterId && removeSet.has(currentLifterId)) {
      setCurrentLifterId(updated[0]?.id ?? null);
    }
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      (() => {
                        ...
                        const parts = (rowForm.team || "").split(" - ");
                        const curState = parts[0] === "India" && parts[1] ? parts[1] : "";
                        const curDistrict = parts[0] === "India" && parts[2] ? parts[2] : "";
                        ...
                        return (
                          <div className="flex min-w-56 flex-col gap-1">
                            ...
                            <div className="flex gap-1">
                              ...
                              <select
                                value={curDistrict}
                                onChange={(e) => updateRow({ team: `India - ${curState} - ${e.target.value}` })}
                                disabled={!curState}
                      ...
                      <div className="flex min-w-52 gap-2">
                        {competitionMode !== "BENCH_ONLY" && (
                          <input
                            ...
                            onChange={(e) => updateRow({ squat1: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="SQ"
                        ...
                        <input
                          ...
                          onChange={(e) => updateRow({ bench1: e.target.value === "" ? "" : Number(e.target.value) })}
                          placeholder="BP"
                          className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                        />
                        {competitionMode !== "BENCH_ONLY" && (
                          <input
                            ...
                            onChange={(e) => updateRow({ deadlift1: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="DL"
                            className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                        )}
...
const GroupManagementPage = () => {
  ...
  const keepAttemptWithFirstWeightLocal = (attempts: Attempt[], firstWeight: number | "") => {
    const next = [...attempts];
    const existingStatus = next[0]?.status;
    const status: AttemptStatus =
      firstWeight === ""
                      ...
                      {isEditing ? (
                        (() => {
                          ...
                          return (
                            <div className="flex min-w-56 flex-col gap-1">
                              <select
                                value={teamOpts.includes(draft.team) ? draft.team : "__custom__"}
                                onChange={(e) => {
                              ...
                              <div className="flex gap-1">
                                <select
                                  ...
                                  onChange={(e) => {
                                    const s = e.target.value;
                                    const d = (INDIA_DISTRICTS[s] ?? [""])[0] ?? "";
                                    updateLifterDraft(l.id, { team: s ? `India - ${s} - ${d}` : "" });
                                  }}
                                  className="h-9 flex-1 rounded-lg border border-white/20 bg-black/40 px-1 text-xs"
                                >
                                  <option value="" className="bg-slate-900">State</option>
                                ...
                                <select
                                  value={curDistrict}
                                  onChange={(e) => updateLifterDraft(l.id, { team: `India - ${curState} - ${e.target.value}` })}
                                  disabled={!curState}
                                  className="h-9 flex-1 rounded-lg border border-white/20 bg-black/40 px-1 text-xs disabled:opacity-40"
                                >
                                  <option value="" className="bg-slate-900">District</option>
                        ...
                        <div className="flex gap-1">
                          {competitionMode !== "BENCH_ONLY" && (
                            <input
                              type="number"
                              value={draft.squat1}
                              onChange={(e) => updateLifterDraft(l.id, { squat1: e.target.value === "" ? "" : Number(e.target.value) })}
                              placeholder="SQ"
                              className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                            />
                          )}
                          <input
                            type="number"
                            value={draft.bench1}
                            onChange={(e) => updateLifterDraft(l.id, { bench1: e.target.value === "" ? "" : Number(e.target.value) })}
                            placeholder="BP"
                            className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                          />
                          {competitionMode !== "BENCH_ONLY" && (
                            <input
                              ...
                              onChange={(e) => updateLifterDraft(l.id, { deadlift1: e.target.value === "" ? "" : Number(e.target.value) })}
                              placeholder="DL"
                              className="h-9 w-16 rounded-lg border border-white/20 bg-black/40 px-2 text-sm"
                            />
                          )}
