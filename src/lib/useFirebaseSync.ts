...
// Keep this short so a remote change (e.g. Control Center marking GOOD/NO LIFT)
// isn't blocked from reaching other screens (Results, Referee, Display).
const PENDING_LOCAL_SESSION_TTL_MS = 800;
      ...
      try {
        ...
        if (!readOnly && pendingLocalSession) {
          if (fingerprint === pendingLocalSession.fingerprint) {
            pendingLocalSessionRef.current = null;
          } else if (Date.now() - pendingLocalSession.updatedAt < PENDING_LOCAL_SESSION_TTL_MS) {
            console.log("[Powerlifting:SessionSync]", "remote session skipped while local save is pending", {
              competitionId: activeCompetitionId,
              reason,
            ...
            console.warn("[Powerlifting:SessionSync]", "pending local save timed out; accepting remote session", {
              competitionId: activeCompetitionId,
              reason,
              pendingSource: pendingLocalSession.source,
            });
