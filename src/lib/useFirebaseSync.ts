...
const SESSION_REFETCH_DEBOUNCE_MS = 0;
const LIFTER_SAVE_DEBOUNCE_MS = 0;
// Cross-client updates (Control ↔ Results, another device) must never be
// blocked behind a TTL — the fingerprint check alone determines whether an
// incoming snapshot is our own echo or a real remote change.
...
export function useFirebaseSync(
  ...
  const pendingLocalSessionRef = useRef<{ fingerprint: string; updatedAt: number; source: string } | null>(null);
  const refetchSessionRef = useRef<((reason: string) => void) | null>(null);

  const refetchSession = useCallback((reason: string = "manual") => {
    const fn = refetchSessionRef.current;
    if (fn) fn(reason);
  }, []);
  ...
  const buildLocalSessionFingerprint = useCallback(() => {
    if (!activeCompetitionId) return;
  ...
  const markLocalSessionFromState = useCallback(() => {
    const fingerprint = buildLocalSessionFingerprint();
    if (!fingerprint) return;
    lastAppliedSessionFingerprintRef.current = fingerprint;
  }, [buildLocalSessionFingerprint]);

  const markPendingLocalSessionFromState = useCallback((source: string) => {
    const fingerprint = buildLocalSessionFingerprint();
    if (!fingerprint) return;
    pendingLocalSessionRef.current = { fingerprint, updatedAt: Date.now(), source };
  }, [buildLocalSessionFingerprint]);

  const { onCompetitionsLoaded, onRefereeSignalsChanged, onDevicesChanged, onCompetitionSessionFromDb } =
    callbacks;

  // ── Realtime load: competitions list + nested lifters/groups ───────────────
  useEffect(() => {
    ...
    const loadFromDb = async () => {
      ...
      console.log("[Powerlifting:SessionSync] loading competitions", {
      ...
      try {
        ...
        console.error("[Powerlifting:SessionSync] competitions load failed", error);
      }
    };

    void loadFromDb();

    const unsub = onValue(ref(firebaseDb, "competitions"), () => {
      void loadFromDb();
    });
  ...
  }, [authLoading, authUserId, onCompetitionsLoaded]);
  ...
  useEffect(() => {
    ...
    markPendingLocalSessionFromState("competition save");
  ...
  }, [readOnly, activeCompetitionId, competitions, markLocalSessionFromState, markPendingLocalSessionFromState]);
  ...
  useEffect(() => {
    ...
    markPendingLocalSessionFromState("lifters save");
  ...
  }, [readOnly, activeCompetitionId, lifters, markLocalSessionFromState, markPendingLocalSessionFromState]);
  ...
  useEffect(() => {
    ...
    markPendingLocalSessionFromState("groups save");
  ...
  }, [readOnly, activeCompetitionId, groups, markLocalSessionFromState, markPendingLocalSessionFromState]);
  ...
  const persistSessionSnapshot = useCallback(
    async (snapshot: SessionPersistSnapshot) => {
      ...
      try {
        pendingLocalSessionRef.current = {
          ...
          updatedAt: Date.now(),
          source: "immediate snapshot persist",
        };
  ...
  useEffect(() => {
    ...
    const fetchAndApplySession = async (reason: string) => {
      try {
        ...
        const pendingLocalSession = pendingLocalSessionRef.current;
        if (!readOnly && pendingLocalSession) {
          // If the incoming snapshot matches our pending fingerprint, our own
          // write just echoed back — clear pending. If it differs, another
          // writer (Control Center vs Results, another device, etc.) made a
          // change and we MUST apply it immediately. Never block cross-client
          // updates behind a TTL: doing so causes Control ↔ Results to stop
          // reflecting each other's attempt edits.
          pendingLocalSessionRef.current = null;
          if (fingerprint !== pendingLocalSession.fingerprint) {
            console.log(
              "[Powerlifting:SessionSync]",
              "remote change superseded pending local save — applying immediately",
              {
                competitionId: activeCompetitionId,
                reason,
                pendingSource: pendingLocalSession.source,
              },
            );
          }
        }
    ...
    // Expose a manual refetch (used by visibility/pageshow handlers) so that
    // iOS Safari bfcache restores and stale-tab wakes immediately reload the
    // current session snapshot instead of waiting for the next Firebase push.
    refetchSessionRef.current = (reason: string) => {
      // Bypass fingerprint gate so a wake always applies the latest snapshot.
      lastAppliedSessionFingerprintRef.current = "";
      pendingLocalSessionRef.current = null;
      void fetchAndApplySession(reason);
    };
    ...
    const handleWake = () => {
      ...
      refetchSessionRef.current?.("wake:visibility/pageshow");
    };
    ...
    return () => {
      ...
      lastAppliedSessionFingerprintRef.current = "";
      if (sessionRefetchTimer) clearTimeout(sessionRefetchTimer);
      refetchSessionRef.current = null;
      if (typeof window !== "undefined") {
